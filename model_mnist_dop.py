...
class HebbLearner():
    ...
    def model(self):
        ...
        with tf.variable_scope('fw'):
            ...
            # third layer
            W_4 = self._weightVar([layer_sizes[2],4],name='W4')
            y_4 = tf.sign(tf.nn.relu(tf.matmul(y_3,W_4)))

            # with tf.variable_scope('bw'):
            #     W_feed_back_1 = self._weightVar([4,layer_sizes[0]],name='W1')
            #     y_feedback_1 = tf.nn.tanh(tf.matmul(tf.cast(labels,tf.float32),W_feed_back_1))

            miss_per_digit = tf.cast(tf.not_equal(y_4,tf.cast(labels,tf.float32)),tf.float32)
    ...
    def train(self,sess,data,labels):
        ...
        for epoch_ind in range(30):
            ...
            for bi in range(int(data.shape[0]/self.batch_size)):
                ...
                W,miss_rate = sess.run([model.backwards_op,model.avg_miss],feed_dict={model.sequences:batch_data,model.labels:batch_labels})
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        hebbLearner.train(sess,pca_train_data,train_label_unpack)
        hebbLearner.test(sess,pca_test_data,test_label_unpack)
